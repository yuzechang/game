GAME_MODULES.lianliankan = (() => {
  ...
  function draw(ts) {
    ...
    cx.fillText(`步数: ${moves}`, 8, 14);
    cx.textAlign = 'right';
    cx.fillStyle = '#a5f3fc';
    cx.fillText('H=提示  S=洗牌', cv.width-8, 14);
    ...
    if (won) {
      ...
      cx.textAlign='center'; cx.textBaseline='middle';
      cx.fillText('通关!', cv.width/2, cv.height/2-12);
      cx.font='10px monospace'; cx.fillStyle='#fff';
      cx.fillText(`用了 ${moves} 步`, cv.width/2, cv.height/2+8);
      cx.fillStyle='#aaa'; cx.font='9px monospace';
      cx.fillText('SPACE 重玩', cv.width/2, cv.height/2+26);
    } else if (over) {
      cx.fillStyle='rgba(0,0,0,0.6)'; cx.fillRect(0,0,cv.width,cv.height);
      cx.fillStyle='#ff5252'; cx.font='bold 14px monospace';
      cx.textAlign='center'; cx.textBaseline='middle';
      cx.fillText('无路可走!', cv.width/2, cv.height/2-12);
      cx.fillStyle='#aaa'; cx.font='9px monospace';
      cx.fillText('S=洗牌  SPACE=重玩', cv.width/2, cv.height/2+12);
    }
