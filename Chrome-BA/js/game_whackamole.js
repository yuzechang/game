GAME_MODULES.mole = (() => {
  ...
  function draw(ts) {
    ...
    if (!running && timeLeft <= 0) {
      ...
      cx.fillStyle = '#aaa';
      cx.fillText('点击或按 SPACE 重玩', cv.width/2, cv.height/2 + 22);
    } else if (!running) {
      cx.fillStyle = '#ffd600';
      cx.font = 'bold 13px monospace';
      cx.textAlign = 'center'; cx.textBaseline = 'middle';
      cx.fillText('打地鼠', cv.width/2, GY - 10);
      cx.font = '9px monospace';
      cx.fillStyle = '#fff';
      cx.fillText('点击或按 SPACE 开始', cv.width/2, cv.height - 12);
    }
